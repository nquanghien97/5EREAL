        ...
        <div className="px-8">
            ...
            <nav className="hidden lg:flex justify-center items-center space-x-8 flex-1 h-full">
              {ListHeader.map((item, index) => (
                <Link
                  key={index}
                  href={item.href}
                  className={`hover:text-[#d29015] transition-colors duration-200 xl:text-base lg:text-xs font-semibold tracking-wide uppercase ${pathname === item.href ? 'text-[#d29015]' : 'text-white'
                    }`}
                >
                  {item.title}
                </Link>
          ...
          >
            <nav className="border-t border-slate-700">
              <div
                className={`flex flex-col space-y-1 py-4 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-y-0' : '-translate-y-4'
                  }`}
              >
                {ListHeader.map((item, index) => (
                  <Link
