...
export async function POST(req: Request) {
  try {
    ...
    const response = await tvly.search(
      `Hãy tìm kiếm thông tin về giá bất động sản mới nhất trong vòng 1 tháng ở ${userMessage}`,
      {
    ...
    await client.beta.threads.messages.create(thread.id, {
      role: "user",
      content: `Hãy format lại dữ liệu bất động sản từ Tavily. 
        Đây là results: ${JSON.stringify(response.results)}`,
    });

    // 4. Tạo run
    const run = await client.beta.threads.runs.create(thread.id, {
      assistant_id: process.env.OPENAI_ASSISTANT_ID!, // Assistant đã setup schema
    });
