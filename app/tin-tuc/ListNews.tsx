...
function ListNews(props: ListNewsProps) {
  ...
  return (
    <section className="mb-4">
      <h2 className="text-2xl font-[800] mb-4 text-center uppercase lg:hidden">Tin tức khác</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.map(news => (
                <Link href={`tin-tuc/${news.slug}`} key={news.id} className="group relative overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                  <Image src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${news.thumbnailUrl}`} alt={news.title} width={600} height={400} className="object-cover w-full h-[320px]" />
